  ...
  */
  public function insert($item = array()){
    if(!is_array($item) OR count($item) === 0){
      ...
      if(!isset($item['id'], $item['name'], $item['price'], $item['qty'], $item['discount'])){
        return FALSE;
      }else{
        /*
        * Insert Item
  ...
  */
  protected function save_cart(){
    ...
    foreach ($this->cart_contents as $key => $val){
      ...
      $this->cart_contents['cart_total'] += ($val['price'] * $val['qty']-$this->cart_contents[$key]['discount']);
      $this->cart_contents['total_items'] += $val['qty'];
      $this->cart_contents[$key]['subtotal'] = ($this->cart_contents[$key]['price'] * $this->cart_contents[$key]['qty']-$this->cart_contents[$key]['discount']);
    }
